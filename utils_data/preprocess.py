...
#z, depth_mhz = 8, 20
depth_mhz = 27.
z = 8.240 #7.683 #8.240, 8.971
#indexes = range(300)
indexes = np.array([232])

path_input = '/store/ska/sk014/serenet/inputs/dataLC_128_pred_190922/'
path_out = '/scratch/snx3000/mibianco/output_serenet/preproc_valid/'
...
for i in tqdm(indexes):
    ...
    t2c.save_cbin(path_out+'dT4pca4_z%.3f_%dMHz_i%d.bin' %(z, depth_mhz, i), data_dTskleanpca[...,idx_i:idx_j+1])
    t2c.save_cbin(path_out+'dT2_z%.3f_%dMHz_i%d.bin' %(z, depth_mhz, i), dT2_sub)
    t2c.save_cbin(path_out+'xHI_z%.3f_%dMHz_i%d.bin' %(z, depth_mhz, i), data_xHI[...,idx_i:idx_j+1])
    t2c.save_cbin(path_out+'xH_z%.3f_%dMHz_i%d.bin' %(z, depth_mhz, i), data_xH[...,idx_i:idx_j+1])
    t2c.save_cbin(path_out+'dT4_z%.3f_%dMHz_i%d.bin' %(z, depth_mhz, i), data_dT4[...,idx_i:idx_j+1])
    np.savetxt(path_out+'redshifts_z%.3f_%dMHz.txt' %(z, depth_mhz), cut_redshift)
    np.savetxt(path_out+'box_dims_z%.3f_%dMHz.txt' %(z, depth_mhz), box_dims, header='in cMpc')
    ...
    t2c.save_cbin('%sdT4poly_z%.3f_%dMHz_i%d.bin' %(path_out, z, depth_mhz, i), dT4poly[...,idx_i:idx_j+1])
    ...
    t2c.save_cbin('%sdT4gpr_z%.3f_%dMHz_i%d.bin' %(path_out, z, depth_mhz, i), dT4gpr)
